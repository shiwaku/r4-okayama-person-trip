...
- 滞在ゾーン判定: 到着時刻 ≤ h:00 < 次の出発時刻（移動中は未カウント）
...
```mermaid
gantt
    title 滞在ゾーン判定のイメージ（1人の行動例）
    dateFormat HH:mm
    axisFormat %H:%M

    section 自宅A
    滞在           :a1, 00:00, 08:00
    帰宅後滞在      :a2, 14:30, 23:59
    ...
    section 移動中（カウント外）
    自宅 → 職場    :crit, m1, 08:00, 08:30
    職場 → 商業    :crit, m2, 12:00, 12:40
    商業 → 自宅    :crit, m3, 13:30, 14:30
```

毎正時スナップショットでの滞在ゾーン判定（到着時刻 ≤ h:00 < 次の出発時刻）:

| 時刻 | 滞在ゾーン | 理由 |
|------|----------|------|
| 6:00 | 自宅A | 00:00 ≤ 6:00 < 8:00（出発前） |
| 8:00 | なし   | 移動中（8:00 出発 → 8:30 到着） |
| 9:00 | 職場B  | 8:30 ≤ 9:00 < 12:00 |
| 12:00 | なし  | 移動中（12:00 出発 → 12:40 到着） |
| 13:00 | 商業C | 12:40 ≤ 13:00 < 13:30 |
| 15:00 | 自宅A | 14:30 ≤ 15:00 < 24:00 |
